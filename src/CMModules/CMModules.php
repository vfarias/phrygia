...
class CMModules extends CObject {
  ...
  * Properties
  */
  private $phrygiaCoreModules = array('CPhrygia', 'CDatabase', 'CRequest', 'CViewContainer', 'CSession', 'CObject');
  private $phrygiaCMFModules = array('CForm', 'CCPage', 'CCBlog', 'CMUser', 'CCUser', 'CMContent', 'CCContent', 'CFormUserLogin', 'CFormUserProfile', 'CFormUserCreate', 'CFormContent', 'CHTMLPurifier');
  ...
  private function GetDetailsOfModule($module) {
    ...
    if(class_exists($module)) {
      ...
      $details['isPhrygiaCore']   = in_array($rc->name, $this->phrygiaCoreModules);
      $details['isPhrygiaCMF']    = in_array($rc->name, $this->phrygiaCMFModules);
